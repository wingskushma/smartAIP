...
class SpreadSheet {
	 ...
	 *		id of sheet
	 */
	public function __construct($connection, $sheetid, $dbconfig) {
		$this->connection = $connection;
		$this->wrapper = new PDODBDataWrapper($connection, null);

		$this->prefix = $dbconfig["prefix"];
		$this->tick = $dbconfig["tick"];
	...
	public function getCell($coord) {
		$cell = new SpreadSheetCell($this->connection, $this->sheetid, $coord, $this->dbconfig);
		return $cell;
	}
	...
	public function getCells() {
		$cells = Array();
		$query = "SELECT {$this->tick}rowid{$this->tick}, {$this->tick}columnid{$this->tick} FROM {$this->prefix}data WHERE {$this->tick}sheetid{$this->tick}='".$this->e($this->sheetid)."'";
		$res = $this->wrapper->query($query);
		while ($coord = $this->wrapper->get_next($res)) {
			$cells[] = new SpreadSheetCell($this->connection, $this->sheetid, $coord, $this->dbconfig);
		}
		return $cells;
	}
...
class SpreadSheetCell {
	 ...
	 *		cell coordinate
	 */
	public function __construct($connection, $sheetid, $coord, $dbconfig) {
		$this->wrapper = new PDODBDataWrapper($connection, null);
	...
	private function load() {
		$query = "SELECT {$this->tick}data{$this->tick}, {$this->tick}parsed{$this->tick}, {$this->tick}calc{$this->tick}, {$this->tick}style{$this->tick} FROM {$this->prefix}data WHERE {$this->where}";
	...
	public function exists($dont_create = false) {
		$query = "SELECT {$this->tick}data{$this->tick} FROM {$this->prefix}data WHERE {$this->where}";
		$result = $this->wrapper->query($query);
		if ($this->wrapper->get_next($result))
			return true;
		else {
			if (!$dont_create) {
				$query = "INSERT INTO {$this->tick}{$this->prefix}data{$this->tick} ({$this->tick}sheetid{$this->tick}, {$this->tick}rowid{$this->tick}, {$this->tick}columnid{$this->tick},{$this->tick}data{$this->tick},{$this->tick}calc{$this->tick},{$this->tick}parsed{$this->tick},{$this->tick}style{$this->tick}) VALUES ('".$this->e($this->sheetid)."', '".$this->e($this->row)."', '".$this->e($this->col)."','','','','{$this->defaultstyle}')";
	...
	protected function process_triggers($triggers) {
		...
		$query = "DELETE FROM {$this->prefix}triggers WHERE {$this->tick}source{$this->tick}='".$this->e($coord)."' AND {$this->tick}sheetid{$this->tick}='".$this->e($this->sheetid)."'";
		$this->wrapper->query($query);
		...
		if ($triggers) {
			$query = "INSERT INTO {$this->prefix}triggers ({$this->tick}sheetid{$this->tick}, {$this->tick}trigger{$this->tick}, {$this->tick}source{$this->tick}) VALUES {$triggers}";
			$this->wrapper->query($query);
		}
	}
	...
	protected function run_triggers() {
		$coord = $this->colLetter.$this->row;
		$query = "SELECT * FROM {$this->prefix}triggers WHERE {$this->tick}trigger{$this->tick}='".$this->e($coord)."' AND {$this->tick}sheetid{$this->tick}='".$this->e($this->sheetid)."'";
		$result = $this->wrapper->query($query);
		$triggers = Array();
		...
		$response = array();
		for ($i = 0; $i < count($triggers); $i++) {
			$cell = new SpreadSheetCell($this->connection, $this->sheetid, $triggers[$i], $this->dbconfig);
	...
	protected function calculateValue() {
		if ((strlen($this->parsed) > 1)&&($this->parsed[0] == '=')) {
			...
			$subcell = new SpreadSheetCell($this->connection, $this->sheetid, $expr, $this->dbconfig);
	...
	public function getCalculatedValue() {
		$query = "SELECT {$this->tick}calc{$this->tick} FROM {$this->prefix}data WHERE {$this->where}";
		$result = $this->wrapper->query($query);
		if (!$result) return false;
		$cell = $this->wrapper->get_next($result);
		if ($cell['calc'] === '')
	...
	public function getParsedValue() {
		$query = "SELECT {$this->tick}parsed{$this->tick} FROM {$this->prefix}data WHERE {$this->where}";
		$result = $this->wrapper->query($query);
		if (!$result) return false;
		$cell = $this->wrapper->get_next($result);
		return $cell['parsed'];
	...
	public function getStyle() {
		$query = "SELECT {$this->tick}style{$this->tick} FROM {$this->prefix}data WHERE {$this->where}";
		$result = $this->wrapper->query($query);
		if (!$result) return false;
		$cell = $this->wrapper->get_next($result);
	...
	public function setStyle($style) {
		...
		$query = "UPDATE {$this->prefix}data SET {$this->tick}style{$this->tick}='".$this->e($style)."' WHERE {$this->where}";
